...
func (tq *TeamQuery) QueryMatch() *MatchQuery {
	query := (&MatchClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TeamQuery) WithMatch(opts ...func(*MatchQuery)) *TeamQuery {
	query := (&MatchClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := tq.withMatch; query != nil {
		if err := tq.loadMatch(ctx, query, nodes, nil,
			func(n *Team, e *Match) { n.Edges.Match = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TeamQuery) loadMatch(ctx context.Context, query *MatchQuery, nodes []*Team, init func(*Team), assign func(*Team, *Match)) error {
...
func (tq *TeamQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withMatch != nil {
			_spec.Node.AddColumnOnce(team.FieldMatchID)
		}
	}
