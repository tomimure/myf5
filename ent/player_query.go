...
func (pq *PlayerQuery) QueryEvents() *EventQuery {
...
func (pq *PlayerQuery) WithEvents(opts ...func(*EventQuery)) *PlayerQuery {
	query := (&EventClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PlayerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Player, error) {
	...
	if query := pq.withEvents; query != nil {
		if err := pq.loadEvents(ctx, query, nodes,
			func(n *Player) { n.Edges.Events = []*Event{} },
			func(n *Player, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlayerQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Player, init func(*Player), assign func(*Player, *Event)) error {
	...
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(player.EventsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.player_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "player_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "player_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
