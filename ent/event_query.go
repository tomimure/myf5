...
func (eq *EventQuery) QueryPlayer() *PlayerQuery {
...
func (eq *EventQuery) QueryMatch() *MatchQuery {
	query := (&MatchClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) Clone() *EventQuery {
	...
	return &EventQuery{
		...
		withPlayer: eq.withPlayer.Clone(),
		withMatch:  eq.withMatch.Clone(),
...
func (eq *EventQuery) WithPlayer(opts ...func(*PlayerQuery)) *EventQuery {
	query := (&PlayerClient{config: eq.config}).Query()
...
func (eq *EventQuery) WithMatch(opts ...func(*MatchQuery)) *EventQuery {
	query := (&MatchClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if eq.withPlayer != nil || eq.withMatch != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := eq.withPlayer; query != nil {
		if err := eq.loadPlayer(ctx, query, nodes, nil,
			func(n *Event, e *Player) { n.Edges.Player = e }); err != nil {
			return nil, err
		}
	}
	if query := eq.withMatch; query != nil {
		if err := eq.loadMatch(ctx, query, nodes, nil,
			func(n *Event, e *Match) { n.Edges.Match = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EventQuery) loadPlayer(ctx context.Context, query *PlayerQuery, nodes []*Event, init func(*Event), assign func(*Event, *Player)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		if nodes[i].player_events == nil {
			continue
		}
		fk := *nodes[i].player_events
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "player_events" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (eq *EventQuery) loadMatch(ctx context.Context, query *MatchQuery, nodes []*Event, init func(*Event), assign func(*Event, *Match)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		if nodes[i].match_events == nil {
			continue
		}
		fk := *nodes[i].match_events
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "match_events" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
